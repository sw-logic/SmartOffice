<script lang="ts">
	...
	let canSubmit = $derived(date !== '' && minutes !== null && minutes > 0);
	...
	async function handleSubmit(e: Event) {
		...
		try {
			const url = isEditing
				? `/api/services/${serviceId}/time-records/${record!.id}`
				: `/api/services/${serviceId}/time-records`;
	...
	async function handleDelete() {
		...
		try {
			const res = await fetch(`/api/services/${serviceId}/time-records/${record.id}`, {
				method: 'DELETE'
			});
		...
		<form onsubmit={handleSubmit} class="space-y-4">
			...
			<div class="grid grid-cols-2 gap-4">
				<div class="space-y-2">
					...
					<Input id="trDate" class="w-full" type="date" bind:value={date} required />
				</div>
				<div class="space-y-2">
			...
			<div class="grid grid-cols-2 gap-4">
				{#if typeOptions.length > 0}
					<div class="space-y-2">
						<Label>Type</Label>
						<Select.Root type="single" value={type || 'none'} onValueChange={(v) => { type = v === 'none' ? '' : v; }}>
				...
				{#if categoryOptions.length > 0}
					<div class="space-y-2">
						...
						<Select.Root type="single" value={category || 'none'} onValueChange={(v) => { category = v === 'none' ? '' : v; }}>
							<Select.Trigger class="w-full">
			...
			<div class="grid grid-cols-2 gap-4">
				<div class="space-y-2">
					<Label>Person</Label>
					<Select.Root type="single" value={userId ? String(userId) : 'none'} onValueChange={(v) => { userId = v === 'none' ? null : parseInt(v); }}>
						<Select.Trigger class="w-full">
