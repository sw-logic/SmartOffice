<script lang="ts">
	...
	let selectedStatus = $state(form?.values?.status || data.vendor.status || defaultStatus);
	let selectedCategory = $state(form?.values?.category || data.vendor.category || '');
	let selectedCurrency = $state(form?.values?.currency || data.vendor.currency || defaultCurrency);
	let selectedPaymentTerms = $state(String(form?.values?.paymentTerms || data.vendor.paymentTerms || 30));
</script>

<div class="space-y-6">
	<div class="flex items-center gap-4">
		<Button variant="ghost" size="icon" href="/vendors/{data.vendor.id}">
	...
	{#if data.vendor.isDeleted}
		<Alert.Root variant="destructive" class="max-w-2xl">
			...
			<Alert.Description>
				This vendor has been deleted. You can edit its details, but it won't appear in normal lists until restored.
			</Alert.Description>
		</Alert.Root>
	{/if}
	...
	>
		<Card.Root class="max-w-2xl">
			<Card.Header>
				...
				<Card.Description>Update the vendor's basic details</Card.Description>
			</Card.Header>
			<Card.Content class="space-y-4">
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						<Input
							...
							value={form?.values?.name || data.vendor.name}
							required
						/>
						{#if form?.errors?.name}
					...
					<div class="space-y-2">
						...
						<Input
							...
							value={form?.values?.companyName || data.vendor.companyName || ''}
						/>
					</div>
				</div>
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						<Input
							...
							value={form?.values?.email || data.vendor.email || ''}
						/>
					...
					<div class="space-y-2">
						...
						<Input
							...
							value={form?.values?.phone || data.vendor.phone || ''}
						/>
					</div>
				</div>
				...
				<div class="space-y-2">
					...
					<Input
						...
						value={form?.values?.website || data.vendor.website || ''}
					/>
		...
		<Card.Root class="max-w-2xl">
			<Card.Header>
				...
				<Card.Description>Vendor's physical address</Card.Description>
			</Card.Header>
			<Card.Content class="space-y-4">
				<div class="space-y-2">
					...
					<Input
						...
						value={form?.values?.street || data.vendor.street || ''}
					/>
				</div>
				...
				<div class="grid grid-cols-3 gap-4">
					<div class="space-y-2">
						...
						<Input
							...
							name="city"
							placeholder="City"
							value={form?.values?.city || data.vendor.city || ''}
						/>
					</div>
					...
					<div class="space-y-2">
						...
						<Input
							...
							name="postalCode"
							placeholder="12345"
							value={form?.values?.postalCode || data.vendor.postalCode || ''}
						/>
					</div>
					...
					<div class="space-y-2">
						...
						<Input
							...
							name="country"
							placeholder="Country"
							value={form?.values?.country || data.vendor.country || ''}
						/>
					</div>
				</div>
			</Card.Content>
			...
			<Card.Content class="space-y-4">
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						<Input
							...
							value={form?.values?.taxId || data.vendor.taxId || ''}
						/>
					</div>
					...
					<div class="space-y-2">
						...
						<Input
							...
							value={form?.values?.vatNumber || data.vendor.vatNumber || ''}
						/>
					</div>
				</div>
				...
				<div class="space-y-2">
					...
					<Textarea
						...
						rows={4}
						value={form?.values?.notes || data.vendor.notes || ''}
					/>
				</div>
			</Card.Content>
		</Card.Root>
		...
		<div class="flex gap-4 max-w-2xl">
			...
			<Button type="button" variant="outline" href="/vendors/{data.vendor.id}">Cancel</Button>
		</div>
	</form>
</div>
