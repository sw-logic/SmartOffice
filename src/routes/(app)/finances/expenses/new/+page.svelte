<script lang="ts">
	...
	let selectedVendor = $state(form?.values?.vendorId || '');
...
<div class="space-y-6">
	<div class="flex items-center gap-4">
		...
		<div>
			<h1 class="text-3xl font-bold tracking-tight">New Expense</h1>
			<p class="text-muted-foreground">Record a new expense entry</p>
		</div>
	</div>
	...
	>
			...
			<Card.Content class="space-y-4">
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						<Select.Root type="single" value={selectedVendor} onValueChange={(v) => { selectedVendor = v; if (v && !selectedPaymentTermDays) { const vn = data.vendors.find(x => String(x.id) === v); if (vn) selectedPaymentTermDays = String(vn.paymentTerms); } }} name="vendorId">
							<Select.Trigger>
		...
		<div class="flex gap-4 max-w-2xl">
			...
			<Button type="button" variant="outline" href="/finances/expenses">Cancel</Button>
		</div>
	</form>
</div>
