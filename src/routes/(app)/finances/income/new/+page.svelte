<script lang="ts">
	...
	let selectedClient = $state(form?.values?.clientId || '');
	...
	let selectedPaymentTermDays = $state(form?.values?.paymentTermDays || '');
	let dateValue = $state(form?.values?.date || new Date().toISOString().split('T')[0]);

	const calculatedDueDate = $derived(() => {
		if (dateValue && selectedPaymentTermDays) {
...
<div class="space-y-6">
	<div class="flex items-center gap-4">
		...
		<div>
			<h1 class="text-3xl font-bold tracking-tight">New Income</h1>
			<p class="text-muted-foreground">Record a new income entry</p>
		</div>
	</div>
	...
	>
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						<Input
							...
							value={form?.values?.invoiceReference || ''}
						/>
					</div>
			...
			<Card.Content class="space-y-4">
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						<Select.Root type="single" value={selectedClient} onValueChange={(v) => { selectedClient = v; if (v && !selectedPaymentTermDays) { const cl = data.clients.find(c => String(c.id) === v); if (cl) selectedPaymentTermDays = String(cl.paymentTerms); } }} name="clientId">
							<Select.Trigger>
		...
		<div class="flex gap-4 max-w-2xl">
			...
			<Button type="button" variant="outline" href="/finances/income">Cancel</Button>
		</div>
	</form>
</div>
