<script lang="ts">
	...
	let selectedUnit = $state(form?.values?.unitOfMeasure || data.item.unitOfMeasure || defaultUnit);
	let isActive = $state(form?.values?.active ?? data.item.active);
</script>
...
<div class="space-y-6">
	<div class="flex items-center gap-4">
		...
		<div>
			<h1 class="text-3xl font-bold tracking-tight">Edit Price List Item</h1>
			<p class="text-muted-foreground">Update {data.item.name}</p>
		</div>
	</div>
			...
			<Card.Content class="space-y-4">
				<div class="grid grid-cols-3 gap-4">
					<div class="col-span-2 space-y-2">
						...
						<Input
							...
							value={form?.values?.name || data.item.name}
							required
						/>
						{#if form?.errors?.name}
					...
					<div class="space-y-2">
						...
						<Input
							...
							value={form?.values?.sku || data.item.sku || ''}
						/>
						{#if form?.errors?.sku}
				...
				<div class="space-y-2">
					...
					<Textarea
						...
						rows={2}
						value={form?.values?.description || data.item.description || ''}
					/>
				</div>
			...
			<Card.Content class="space-y-4">
				<div class="grid grid-cols-3 gap-4">
					<div class="space-y-2">
						...
						<Input
							...
							min="0"
							placeholder="0.00"
							value={form?.values?.unitPrice || Number(data.item.unitPrice)}
							required
						/>
				...
				<div class="space-y-2">
					...
					<Input
						...
						placeholder="0"
						class="w-32"
						value={form?.values?.taxRate || (data.item.taxRate ? Number(data.item.taxRate) : '')}
					/>
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						<Input
							...
							name="validFrom"
							type="date"
							value={form?.values?.validFrom || formatDateForInput(data.item.validFrom)}
						/>
					</div>
					...
					<div class="space-y-2">
						...
						<Input
							...
							name="validTo"
							type="date"
							value={form?.values?.validTo || formatDateForInput(data.item.validTo)}
						/>
			...
			<Card.Content class="space-y-4">
				<div class="space-y-2">
					...
					<Textarea
						...
						rows={3}
						value={form?.values?.notes || data.item.notes || ''}
					/>
				</div>
			</Card.Content>
		</Card.Root>
