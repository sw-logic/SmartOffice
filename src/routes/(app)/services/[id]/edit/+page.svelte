<script lang="ts">
	...
	// Select state
	let selectedClientId = $state(data.service.clientId ? String(data.service.clientId) : '');
	let selectedContactId = $state(data.service.contactId ? String(data.service.contactId) : '');
	let selectedAssignedToId = $state(data.service.assignedToId ? String(data.service.assignedToId) : '');
	let selectedType = $state(data.service.type || '');
	let selectedStatus = $state(data.service.status || 'active');
	let selectedCurrency = $state(data.service.currency || 'HUF');
	let selectedPeriod = $state(data.service.recurringPeriod || 'monthly');
	let budgetedHours = $state<number | null>(data.service.budgetedHours ?? null);
...
<div class="space-y-6">
	<div class="flex items-center gap-4">
		<Button variant="ghost" size="icon" href="/services/{data.service.id}">
			<ArrowLeft class="h-4 w-4" />
		</Button>
		<div>
			<h1 class="text-3xl font-bold tracking-tight">Edit Service</h1>
			<p class="text-muted-foreground">Update service details</p>
		</div>
	</div>
	...
	>
				...
				<div class="grid grid-cols-2 gap-4">
					<div class="space-y-2">
						...
						<Input
							...
							name="startDate"
							type="date"
							value={toDateStr(data.service.startDate)}
							required
						/>
					</div>
					...
					<div class="space-y-2">
						...
						<Input
							...
							name="endDate"
							type="date"
							value={toDateStr(data.service.endDate)}
						/>
					</div>
				</div>
			</Card.Content>
		...
		<div class="flex gap-4 max-w-2xl">
			...
			<Button type="button" variant="outline" href="/services/{data.service.id}">Cancel</Button>
		</div>
	</form>
</div>
